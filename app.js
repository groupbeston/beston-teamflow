...
function renderSupervisorDashboard() {
  ...
  document.getElementById('stat-todo-tasks').textContent = todo;
  document.getElementById('stat-progress-tasks').textContent = progress;
  document.getElementById('stat-blocked-tasks').textContent = blocked;
...
// LINE SHARING
// --------------------------------------------------------------------------
function sendLineAlert(taskId) {
  const task = state.tasks.find(t => t.id === taskId); if(!task) return;
  const member = state.teamMembers[task.assignee]||{name:'ทีมงาน'};
  const urg = getDeadlineUrgency(task.deadline,task.status);
  const msg=`[TeamFlow] 📋 ติดตามงาน
━━━━━━━━━━━━━━━━━
📌 ${task.title}
👤 ผู้รับผิดชอบ: ${member.name.split(' ')[0]}
📅 กำหนดส่ง: ${task.deadline}
📊 สถานะ: ${getStatusThai(task.status)}
⚡ ${urg.label}
━━━━━━━━━━━━━━━━━
กรุณาอัพเดทสถานะงานด้วยนะครับ/ค่ะ 🙏`;
  window.open(`https://line.me/R/msg/text/?${encodeURIComponent(msg)}`,'_blank');
  addSecurityLog('line-alert',`ส่ง Line ติดตามงาน "${task.title.substring(0,20)}..."`);
  showToast(`เปิด Line ติดตามงาน ${member.name.split(' ')[0]} แล้ว!`);
}
...
function sendLineSummary() {
  ...
  if(!urgent.length){showToast('ไม่มีงานด่วนในขณะนี้!','error');return;}
